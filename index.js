...
const limiter = new Bottleneck({ minTime: 2000 }); // Reduce CPU spikes by limiting API calls
...
async function forwardPort(name, internalPort, externalPort, protocol = "TCP") {
  return new Promise(async (resolve, reject) => {
    client.portMapping(
      { public: externalPort, private: internalPort, protocol, ttl: 3600 },
      async err => {
        if (err) {
          console.error(`Failed to forward port ${externalPort}:`, err.message);
          return reject(err);
        }
        console.log(`Port ${externalPort} forwarded successfully.`);

        const existingMapping = await PortMapping.findOne({
          where: { externalPort, protocol }
        });

        if (!existingMapping) {
          await PortMapping.create({ name, internalPort, externalPort, protocol });
        }
        resolve();
      }
    );
...
async function isPortOpen(host, port, protocol = "TCP", timeout = 3000) {
  try {
    const response = await fetch("https://api.nekosunevr.co.uk/v5/proxy/portchecker", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ host, port: Number(port), protocol, timeout })
    });
...
async function retryForwardPort(name, internalPort, externalPort, protocol, retries) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      await forwardPort(name, internalPort, externalPort, protocol);
      console.log(`Successfully forwarded port ${externalPort} on attempt ${attempt}`);
      return;
    } catch (err) {
      console.warn(`Attempt ${attempt} to forward port ${externalPort} failed:`, err.message);
      if (attempt === retries) {
        console.error(`Failed to forward port ${externalPort} after ${retries} attempts.`);
      } else {
        await new Promise(resolve => setTimeout(resolve, 2000)); // Avoid instant retry
      }
    }
  }
}
